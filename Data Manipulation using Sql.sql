...
alter table branch add city varchar(50);
select * from branch;
...
select concat(fname,' ',mname,' ',lname)Full_Name from customer where concat(fname,' ',mname,' ',lname) like '_a%';

-- 10.	Find the lowest balance from customer and account table

select c.cust_id,fname,mname,lname,occupation,bid,min(curbal),atype,opndt,astatus from customer c join accounts a 
on c.cust_id=a.cust_id;

-- 11.	Give the count of customer for each occupation
select fname,mname,lname,count(cust_id)C_customers,occupation
from customer
group by occupation;

-- 12.	Write a query to find the name (first_name, last_name) of the employees who are managers.
select e1.fname,e1.lname,e2.fname,e2.lname from employee e1 join employee e2
where e2.mngr_no=e1.emp_no ;

-- 13.	List name of all employees whose name ends with a
select concat(fname,' ',mname,' ',lname)full_name from employee where concat(fname,' ',mname,' ',lname) like '%a';
...
-- 15.	Write a query to display the customer number, customer firstname, account number for the  customer’s who are born after 15th of any month.
 select c.cust_id,fname,acnumber from customer c join 
 accounts a
 on c.cust_id=a.cust_id
 where day(dob)>15;
 ...
 select c.cust_id,c.fname,a.bid,a.curbal from customer c join accounts a
 using(cust_id);
...
# adding some values in branch and customer tables
...
alter table customer rename column city to city_of_residency;

create view cust as
(select c.*,b.name as branch_city from customer c
join branch b
on b.name=c.city_of_residency);
select * from cust;
...
#DOT -  date of transaction
 ...
 bid int,
 amount float,
 t_type varchar(100) check(t_type in('Withdraw','deposit')),
 DOT datetime,
 primary key(TID),
...
for each row
begin
if new.t_type = 'Deposit' then update accounts set curbal = curbal+new.amount where cust_id=new.cust_id ;
elseif new.t_type = 'Withdraw' then update accounts set curbal = curbal-new.amount where cust_id=new.cust_id ;
end if;
end // 
DELIMITER ;
...
select fname,t.cust_id,acnumber,atype,astatus,curbal from customer c
join
(select cust_id,acnumber,atype,astatus,curbal from accounts order by curbal desc limit 1,1)t
